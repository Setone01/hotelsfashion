...
import { TrendingBrand } from "../Assets/dummyDB";

const Brand = () => {
  return (
    <div className="w-screen max-h-max py-10">
      <div className="w-[90%] mx-auto flex justify-start flex-col ">
        <div className="">
          <SecctionHeader
        ...
        <div className="grid w-full pb-7 [@media(min-width:350px)]:grid-cols-[repeat(auto-fill,_minmax(200px,_1fr))] [@media(min-width:768px)]:grid-cols-[repeat(auto-fill,_minmax(280px,_1fr))] gap-6">
          {TrendingBrand &&
            TrendingBrand.map((item) => (
              <BrandCard
                key={item.id}
                image={item.image}
                title={item.title}
